  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">My Workout Programs</h1>
        <Link href="/dashboard/search" className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700">
          + Create New
        </Link>
      </div>
      ...
      {programs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {programs.map((program) => (
            <Link 
              key={program.id} 
              href={`/dashboard/programs/${program.id}`} // Link ke halaman detail
